document.addEventListener('DOMContentLoaded', () => {
    ...
    // Node class (connection points along wires)
    class Node {
        constructor(x, y) {
            this.x = x;
            this.y = y;
            this.radius = 4; // Fixed size for nodes
        }
    ...
    class Wire {
        constructor(x) {
            this.x = x;
            this.nodes = [];
            // Place nodes evenly along the wire
            for (let i = 1; i <= nodesPerWire; i++) {
                this.nodes.push(new Node(this.x, i * nodeSpacing));
            }
        }
